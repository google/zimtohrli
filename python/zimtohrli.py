...
class Zimtohrli:
  ...
  frequency_norm_order: signal.Numerical = 4
  time_norm_order: signal.Numerical = 4
  ...
  def distance(
      self, spectrogram_a: signal.Channels, spectrogram_b: signal.Channels
  ) -> signal.Numerical:
    ...
    return jnp.linalg.norm(
        jnp.linalg.norm(
            jnp.abs(spectrogram_a.samples - spectrogram_b.samples),
            axis=0,
            order=self.frequency_norm_order,
        ),
        order=self.time_norm_order,
    )
