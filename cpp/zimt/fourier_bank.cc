...
void Rotators::FilterAndDownsample(hwy::Span<const float> in,
  ...
  std::vector<float> weights(downsampling);
  for (int i = 0; i < downsampling; ++i) {
    weights[i] = 1.0 / (1.0 + exp(7.944646 * ((2.0 / downsampling) * (i + 0.5) - 1)));
  }
  Resonator resonator;
  size_t out_ix = 0;
  for (int64_t in_ix = 0; in_ix < in.size() && out_ix < out.shape()[0]; ++out_ix) {
    OccasionallyRenormalize();
    for (int64_t zz = 0; zz < downsampling && in_ix < in.size(); ++zz, ++in_ix) {
...
float Freq(int i) {
  // Center frequencies of the filters in the filter bank, plus one
  // frequency in both ends for bandwidth calculation.
  static const float kFreq[130] = {
    ...
    771.921, 806.905, 843.111, 880.584, 919.366, 959.503, 1001.044, 1044.03, 1088.53,
