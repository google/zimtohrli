...
class PyohrliTest(unittest.TestCase):
  ...
  def test_getters_setters(self):
    metric = pyohrli.Pyohrli(sample_rate=48000.0, frequency_resolution=4)
    ...
    perceptual_sample_rate = metric.perceptual_sample_rate
    self.assertEqual(perceptual_sample_rate, 100)
    metric.perceptual_sample_rate *= 2
    self.assertEqual(metric.perceptual_sample_rate, perceptual_sample_rate * 2)
  ...
  @parameterize(
      ...
      dict(
          ...
          a_hz=5000.0,
          b_hz=5010.0,
          distance=1.4676928520202637,
      ),
      dict(
          testcase_name='large',
          a_hz=5000.0,
          b_hz=10000.0,
          distance=55.76076889038086,
      ),
  )
